...
CREATE OR REPLACE FUNCTION _cm3_load_cached_records(_function regproc) RETURNS VOID AS $$ BEGIN
    PERFORM _cm3_load_cached_records(_function, 43200);
END $$ LANGUAGE PLPGSQL;

CREATE OR REPLACE FUNCTION _cm3_renew_cached_records(_function regproc) RETURNS VOID AS $$ BEGIN
    PERFORM _cm3_renew_cached_records(_function, 43200, 43200 / 2);
END $$ LANGUAGE PLPGSQL;

CREATE OR REPLACE FUNCTION _cm3_renew_cached_records(_function regproc, _ttl bigint, _if_older_than bigint) RETURNS VOID AS $$ DECLARE
...
BEGIN
    IF NOT EXISTS (SELECT * FROM "_Cache" WHERE "Code" = _code AND "BeginDate" + format('%s seconds', _if_older_than)::interval > now() ) THEN
        PERFORM _cm3_load_cached_records(_function, _ttl);
    END IF;
END $$ LANGUAGE PLPGSQL;

CREATE OR REPLACE FUNCTION _cm3_load_cached_records(_function regproc, _ttl bigint) RETURNS VOID AS $$ DECLARE
    _code varchar = format('CM_FUNCTION%s', _function::regproc::varchar); 
BEGIN
...
CREATE OR REPLACE FUNCTION _cm3_get_cached_records(_function regproc) RETURNS SETOF RECORD AS $$ DECLARE
    _code varchar = format('CM_FUNCTION%s', _function::regproc::varchar);
...
BEGIN
    IF NOT EXISTS (SELECT * FROM "_Cache" WHERE "Code" = _code AND "BeginDate" + format('%s seconds', "TimeToLive")::interval > now() ) THEN
        ...
        IF NOT EXISTS (SELECT * FROM "_Cache" WHERE "Code" = _code AND "BeginDate" + format('%s seconds', "TimeToLive")::interval > now() ) THEN
            PERFORM _cm3_load_cached_records(_function);
        END IF;
    END IF;
...
CREATE OR REPLACE FUNCTION _fix_navtree_element(_navtree jsonb) RETURNS jsonb AS $$ DECLARE
_target varchar;
BEGIN
    ...
    ELSE
        ...
        END IF;
    END IF;
    _navtree = _navtree || jsonb_build_object('nodes', (SELECT coalesce(jsonb_agg(fixed_node),'[]'::jsonb) FROM ( SELECT _fix_navtree_element(x) fixed_node FROM jsonb_array_elements(_navtree->'nodes') x WHERE jsonb_typeof(x) <> 'null') fixed_nodes));
    RETURN _navtree;
END $$ LANGUAGE PLPGSQL;

-- COMMENT TYPE: function
CREATE OR REPLACE FUNCTION _cm3_invert_navtree_nodes_directions(navcode varchar) RETURNS TABLE (_test varchar) AS $$ BEGIN
    IF _cm3_utils_operation_role_get() = 'SuperUser' THEN
        UPDATE "_NavTree" SET "Data" = _fix_navtree_element("Data") WHERE "Code" = navcode AND "Status" = 'A';
    ELSE
