...
CREATE OR REPLACE FUNCTION _cm3_attribute_validate_features_and_type(_comment jsonb, _type text) RETURNS VOID AS $$ BEGIN
	IF (SELECT EXISTS (SELECT * FROM jsonb_each_text(_comment) WHERE key IN ('REFERENCEDOM','LOOKUP','FKTARGETCLASS') AND coalesce(value,'') <> '' )) AND _type NOT IN ('int8','bigint','varchar','int8[]','bigint[]','varchar[]') THEN
...
CREATE OR REPLACE FUNCTION _cm3_cards_delete(_class regclass, _where_expr varchar) RETURNS void AS $$ BEGIN
	...
	ELSE
		...
		EXECUTE format('UPDATE %s SET "Status" = ''N'' WHERE %s', _class, _where_expr);
	END IF;
END $$ LANGUAGE PLPGSQL;
...
CREATE OR REPLACE FUNCTION _cm3_cards_update(_class regclass, _update_expr varchar, _where_expr varchar) RETURNS void AS $$ BEGIN
	...
	ELSE
		...
		EXECUTE format('UPDATE %s SET %s WHERE "Status" = ''A'' AND %s', _class, _update_expr, _where_expr);
	END IF;
END $$ LANGUAGE PLPGSQL;
